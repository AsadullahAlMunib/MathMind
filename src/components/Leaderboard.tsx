  ...
  return (
    <div className="max-w-3xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4">
      <div className="text-center space-y-2">
        ...
        <p className="opacity-60">{language === 'en' ? 'Your personal bests across all difficulties' : 'সব ধরনের ডিফিকাল্টিতে আপনার সেরা স্কোরসমূহ'}</p>
      </div>
      ...
      <section className="math-card text-center p-12 overflow-hidden relative">
...
function ScoreTile({ label, score, icon, color, rank }: { label: string, score: number, icon: React.ReactNode, color: string, rank: number }) {
  ...
  return (
    <motion.div
      whileHover={{ x: 10 }}
      className="math-card p-2 flex items-center gap-6 group pr-10"
    >
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center bg-opacity-10 ${colors[color]}`}>
        {React.cloneElement(icon as React.ReactElement, { size: 32 })}
      </div>
      
      <div className="flex-1">
         <h3 className="text-sm font-bold uppercase tracking-widest opacity-40">{label}</h3>
         <p className="text-3xl font-black">{score}</p>
      </div>

      <div className="hidden md:flex flex-col items-end opacity-20 group-hover:opacity-100 transition-opacity">
         <span className="text-[10px] font-black uppercase tracking-tighter">Your Rank</span>
