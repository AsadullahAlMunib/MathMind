    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="opacity-60">{language === 'en' ? 'Unlock new visual styles with your points!' : 'আপনার পয়েন্ট দিয়ে নতুন ভিজ্যুয়াল স্টাইল আনলক করুন!'}</p>
        </div>
        <div className="bg-amber-500/10 text-amber-500 px-6 py-3 rounded-2xl flex items-center gap-3 font-bold text-xl border border-amber-500/20">
          <Coins size={24} />
          {totalPoints}
        </div>
      </div>
        ...
        {THEMES.map((theme) => {
          ...
          return (
            ...
            >
              ...
              {/* Theme Preview */}
              <div 
                className="w-full h-32 rounded-2xl flex items-center justify-center shadow-inner overflow-hidden relative"
                style={{ backgroundColor: theme.colors.bg }}
              >
                <div className="space-y-2 flex flex-col items-center">
                  <div className="w-20 h-4 rounded-full" style={{ backgroundColor: theme.colors.primary }}></div>
                  <div className="w-12 h-4 rounded-full" style={{ backgroundColor: theme.colors.secondary, opacity: 0.5 }}></div>
                  <div className="text-xs font-bold" style={{ color: theme.colors.text }}>Sample Text</div>
                </div>
                {!isUnlocked && (
